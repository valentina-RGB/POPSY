...
const RolList: React.FC = () => {
  ...
  const handleToggleEstado = async (id: number, estadoActual: string) => {
    ...
    try {
      // Intenta actualizar el estado en el servidor
      await api.put(`/roles/${id}`, { estado_rol: nuevoEstado });

      // Actualiza el estado en el cliente después de la actualización exitosa en el servidor
      setRol((prevRoles) =>
        prevRoles.map((rol) =>
          rol.id_rol === id ? { ...rol, estado_rol: nuevoEstado } : rol
        )
      );
  ...
  const columns = useMemo<MRT_ColumnDef<Rol>[]>(() => [
    {
      accessorKey: 'id_rol', // Asegúrate de que este campo coincide con el objeto en el servidor
      header: 'ID',
    },
    {
    ...
    },
    {
      ...
      header: 'Estado',
      Cell: ({ cell, row }) => {
        const estadoActual = cell.getValue<string>();
        return (
          <div className="tw-flex tw-items-center">
            <span
              className={`tw-inline-block tw-text-xs tw-font-semibold tw-rounded-full tw-py-1 tw-px-2 ${
                estadoActual === 'D' ? 'tw-bg-green-100 tw-text-green-800' : 'tw-bg-red-100 tw-text-red-800'
              }`}
            >
              {estadoActual === 'D' ? 'Activo' : 'Inactivo'}
            </span>
            <button
              onClick={() => handleToggleEstado(row.original.id_rol, estadoActual)}
              className="tw-ml-2 tw-text-gray-700 tw-transition-colors hover:tw-text-gray-900"
            >
              <FontAwesomeIcon
                icon={estadoActual === 'D' ? faToggleOn : faToggleOff}
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div className="tw-flex tw-justify-center tw-gap-2">
          <button
            onClick={() => handleEdit(row.original.id_rol)}
